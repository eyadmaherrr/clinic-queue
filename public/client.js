...
function initDashboardPage() {
  ...
  const currentServingDisplay = document.getElementById(
  ...
  if (addPatientForm) {
    addPatientForm.addEventListener("submit", async (e) => {
      ...
      const area =
        document.getElementById("patientArea")?.value.trim() || "Unknown";
      const lastVisitDate =
        document.getElementById("lastVisitDate")?.value || null;
      const isNewPatient =
      ...
      try {
        const response = await fetch("/api/add-patient", {
          ...
          body: JSON.stringify({
            ...
            isPriority,
            area,
            birthDate, // Include birth date in the request
            lastVisitDate,
            isNewPatient,
          }),
        });
        ...
        if (data.success) {
          ...
          document.getElementById("birthDate").value = ""; // Clear birth date
  ...
  if (phoneInput) {
    phoneInput.addEventListener("blur", async function () {
      ...
      try {
        ...
        // Inside the phone input blur handler, where you handle returning patients
        if (data.found) {
          ...
          // Clear birth date for new patients
          const birthDateInput = document.getElementById("birthDate");
          if (birthDateInput) {
            birthDateInput.value = "";
            birthDateInput.readOnly = false;
        ...
        if (visitHint) {
          visitHint.textContent =
            "❌ Error checking history. Please try again.";
  ...
  window.removePatient = (patientId) => {
    if (
      confirm("Are you sure you want to remove this patient from the queue?")
    ) {
  ...
  window.togglePriority = (patientId) => {
    fetch("/api/toggle-priority", {
      ...
      .then((data) => {
        if (data.success) {
          showNotification(
            data.isPriority ? "⭐ Priority ON" : "Priority OFF",
            "success",
          );
        }
      })
  ...
  window.markMissed = (patientId) => {
    if (
      confirm(
        "Mark this patient as missed? They will turn gray and be skipped.",
      )
    ) {
  ...
  socket.on("queue-update", (data) => {
    ...
    if (totalWaitTime) {
      const totalWait = data.queue.reduce(
        (sum, patient) => sum + (patient.waitingTime || 0),
        0,
      );
    ...
    if (priorityCount) {
      const priorityPatients = data.queue.filter(
...
function renderQueueList(queue) {
  ...
  if (queue.length === 0) {
    queueList.innerHTML =
      '<div class="empty-queue-message">No patients in queue</div>';
    return;
  }
...
function initScreenPage() {
  ...
  socket.on("queue-update", (data) => {
    if (nowServing) {
      ...
      if (data.currentServing) {
        nowServing.textContent = data.currentServing.id
          .toString()
          .padStart(3, "0");
        if (nowServingName) {
    ...
    if (nextPatients) {
      if (nextEight.length === 0) {
        ...
        nextPatients.innerHTML = nextEight
          .map((patient, index) => {
            let patientClass = "";
...
function initTrackPage() {
  ...
  function updatePatientDisplay(data) {
    ...
    if (progressBar && data.queueLength > 0 && data.position) {
      const progress =
        ((data.queueLength - data.position) / data.queueLength) * 100;
  ...
  function setupSocketForTrack() {
    ...
    socketConnection.on("queue-update", (data) => {
      ...
      if (patient) {
        ...
        if (
          data.currentServing &&
          data.currentServing.id === currentPatientId
        ) {
          const currentServingEl = document.getElementById("currentServing");
          if (currentServingEl) {
            currentServingEl.innerHTML = `<span style="color: var(--success); font-weight: 700;">#${data.currentServing.id} - ${data.currentServing.name} (YOU ARE NOW BEING SERVED)</span>`;
          }
...
function initDoctorPage() {
  ...
  socket.on("queue-update", (data) => {
    ...
    if (data.currentServing) {
      ...
      if (nowServingArea)
        nowServingArea.textContent = data.currentServing.area || "Unknown";
      if (nowServingStatus) {
        nowServingStatus.textContent = data.currentServing.isPriority
          ? "⭐ Priority"
          : "Regular";
        nowServingStatus.className = data.currentServing.isPriority
          ? "priority"
          : "";
      }
    } else {
    ...
    if (nextPatients) {
      if (nextFive.length === 0) {
        nextPatients.innerHTML =
          '<div class="empty-next">No patients waiting</div>';
      } else {
        nextPatients.innerHTML = nextFive
          .map(
            (patient, index) => `
  ...
  fetch(`/api/patients/${patientId}`)
    ...
    .then((data) => {
      ...
      document.getElementById("modalPatientPhone").textContent =
        patient.phone_digits ? "+" + patient.phone_digits : "-";
      document.getElementById("modalPatientArea").textContent =
        patient.area || "-";
      document.getElementById("modalFirstVisit").textContent = formatDate(
        patient.first_visit_date,
      );
      document.getElementById("modalLastVisit").textContent = formatDate(
        patient.last_visit_date,
      );
      document.getElementById("modalTotalVisits").textContent =
